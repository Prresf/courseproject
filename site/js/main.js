/**
 * main.js
 * Компьютерные языки разметки — курсовой проект
 * Управление корзиной, DOM, формами и навигацией
 */
...
function renderCartUI() {
  ...
  drawerLists.forEach(list => {
    if (cart.length === 0) {
      list.innerHTML = '<p style="text-align:center;color:#655f4c;padding:2rem;">Корзина пуста</p>';
    } else {
  ...
  if (checkoutItems) {
    if (cart.length === 0) {
      checkoutItems.innerHTML = '<p style="color:#655f4c;">Ваша корзина пуста. <a href="catalog.html" style="color:#974362;text-decoration:underline;">Перейти в каталог</a></p>';
    } else {
...
function initCatalogFilters() {
  ...
  const checkboxes = document.querySelectorAll('.catalog__filters input[type="checkbox"]');
...
function initAddButtons() {
  document.addEventListener('click', event => {
    ...
    const name = button.dataset.name || button.closest('article')?.querySelector('h3')?.textContent || 'Товар';
    const price = button.dataset.price || button.closest('article')?.querySelector('.price')?.textContent?.replace(/[^0-9.]/g, '') || '0';
...
function initProductPage() {
  ...
  addButton.addEventListener('click', () => {
    ...
    addToCart(addButton.dataset.id, addButton.dataset.name, addButton.dataset.price, quantity);
    const drawer = document.getElementById('cart-drawer');
    if (drawer) {
      drawer.classList.add('is-open');
      drawer.setAttribute('aria-hidden', 'false');
...
function initCheckoutForm() {
  ...
  document.querySelectorAll('.payment-option').forEach(option => {
    option.addEventListener('click', () => {
      document.querySelectorAll('.payment-option').forEach(optionElement => optionElement.classList.remove('is-selected'));
